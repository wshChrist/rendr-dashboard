...
export function TransactionTable({ data }: TransactionTableProps) {
  ...
  // Fonction de filtrage global personnalisée pour gérer les objets complexes
  const globalFilterFn = React.useCallback(
    (row: Row<Transaction>, columnId: string, filterValue: any) => {
      ...
      // Recherche dans les différentes colonnes
      const pair = String(original.pair || '').toLowerCase();
      const brokerName = original.broker?.name
        ? String(original.broker.name).toLowerCase()
        : '';
      const volume = String(original.volume || '');
      const commission = String(original.commission || '');
      const cashback = String(original.cashback_amount || '');
      const status = String(original.status || '').toLowerCase();
      const tradeId = original.trade_id
        ? String(original.trade_id).toLowerCase()
        : '';

      return (
        pair.includes(searchValue) ||
        brokerName.includes(searchValue) ||
        volume.includes(searchValue) ||
        commission.includes(searchValue) ||
        cashback.includes(searchValue) ||
        status.includes(searchValue) ||
        tradeId.includes(searchValue)
      );
    },
    []
  );
